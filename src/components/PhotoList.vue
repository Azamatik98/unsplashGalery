<template>
    <section class="section section-grid">
        <div class="container">
            <div class="loader" v-if="photo.loadState === partials.LOAD_STATE.LOADING
                "></div>

            <div class="grid" v-if="searchPhoto.photos.length <= 0 &&
                photo.loadState === partials.LOAD_STATE.SUCCESS
                ">
                <PhotoItem v-for="photo in photo.photos" :key="photo.id" :photo="photo" />
            </div>

            <div class="grid" v-if="searchPhoto.photos.length > 0 &&
                searchPhoto.loadState === partials.LOAD_STATE.SUCCESS
                ">
                <PhotoItem v-for="photo in searchPhoto.photos" :key="photo.id" :photo="photo" />
            </div>
        </div>
    </section>
</template>
